...
public:
    ...
    int get_max_value2(vector<int>& position, vector<int>& radius, vector<int>& value){
        int n = position.size();
    ...
    void get_max_value(vector<int>& position, vector<int>& radius, vector<int>& value){
        int n = position.size();
        vector<pair<int, int>> vec;
        for(int i = 0; i < n; ++i){
            vec.push_back({i, value[i]});
        }
        ...
        int temp = get_max_value2(position, radius, value);
        ans = max(ans, temp);
        cout<<ans;
    }
};
