...
如果图 GG 中存在环（即图 GG 不是「有向无环图」），那么图 GG 不存在拓扑排序。这是因为假设图中存在环 x_1, x_2, \cdots, x_n, 
，那么 x_1在排列中必须出现在 x_n的前面，但 x_n同时也必须出现在 x_1的前面，因此不存在一个满足要求的排列，也就不存在拓扑排序；
...
假设我们当前搜索到了节点 u，如果它的所有相邻节点都已经搜索完成，那么这些节点都已经在栈中了，此时我们就可以把 u 入栈。可以发现，如果我们从栈顶往栈底的顺序看，由于 u 处于栈顶的位置，
...
如果某个相邻节点变成了「没有任何入边的节点」，那么就代表着这门课可以开始学习了。按照这样的流程，我们不断地将没有入边的节点加入答案，直到答案中包含所有的节点（得到了一种拓扑排序）
或者不存在没有入边的节点（图中包含环）。
...
class Solution{
private:
    vector<vector<int>> edges;//存储有向图，邻接矩阵
    vector<int> visited;//标记每个节点的状态，0未搜索，1搜索中，2已搜索
    vector<int> result;//数组模拟栈，减少时间空间复杂度
...
public:
    vector<int> findOrder(int numCourses, vector<vector<int>>& prerequisites){
        ...
        for(const auto& item : prerequisites){
            edges[item[1]].push_back(item[0]);//使用邻接矩阵表示图
        }

        for(int i = 0; i < numCourses && valid; ++i){//遍历所有未搜索的节点，寻找一条可行的拓扑排序,
            if(visited[i] == 0){
                dfs(i);
            }
        }
